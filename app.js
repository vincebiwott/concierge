...
function initWelcome(){
  ...
  document.getElementById('w-greet').textContent=greet+', '+CU.full_name.split(' ')[0]+' '+String.fromCodePoint(0x1F44B);
...
function pg(id,btn){
  ...
  var grOk=(id==='complaints'||id==='lostfound');
  if(pgActions[id]&&CR!=='gm'&&(!isGR()||grOk)){ab.classList.remove('hide');ab.textContent=pgActions[id][0];ab.onclick=function(){openM(pgActions[id][1]);};}
...
async function loadNotifs(){
  ...
  if(!isGR()){
    ...
    if(ol.data&&ol.data.length)ol.data.forEach(function(r){notifs.push({type:'warn',icon:'X',text:'Overdue luggage: '+r.guest_name+' (Tag: '+(r.tag_code||'-')+')',time:fd(r.time_in)});});
  }
  ...
  var nb=document.getElementById('notif-body'),dot=document.getElementById('notif-dot'),cnt=document.getElementById('notif-count');
...
async function loadDash(){
  ...
  if(!isGR()){
    ...
    if(!r.data||!r.data.length){ra.innerHTML='<div class="es"><div class="ei">!</div><p>No recent activity</p></div>';}
    else{ra.innerHTML=r.data.map(function(x){return '<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border);"><div style="width:32px;height:32px;border-radius:8px;background:var(--sage);display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;">B</div><div style="flex:1;min-width:0;"><div style="font-size:12px;font-weight:600;">'+x.guest_name+' - Room '+(x.room_number||'-')+'</div><div style="font-size:11px;color:var(--ink3);">Tag: <span class="tag-code" style="font-size:10px;">'+(x.tag_code||'-')+'</span> · '+x.handled_by+' · '+fd(x.time_in)+'</div></div><span class="badge '+(x.status==='stored'?'bg':'bgr')+'">'+x.status+'</span></div>';}).join('');}
  } else {
    if(ra)ra.innerHTML='<div class="es"><div class="ei">G</div><p>Guest Relations view</p></div>';
  }
  var shifts=await db.from('shifts').select('*').eq('date',today).order('created_at',{ascending:false}).limit(4);
  var ss=document.getElementById('shift-summary'),sd=document.getElementById('shift-display');
...
function renderSplit(rows){
  ...
  if(!ins.length){ci.innerHTML='<div class="es" style="padding:24px;"><div class="ei">B</div><p>No bags in storage</p></div>';}
  else{
    ci.innerHTML=ins.map(function(r){
      var od=isLugOD(r);var canDel=CR==='admin';
      ...
      +(r.photo_url?'<img src="'+r.photo_url+'" style="width:44px;height:44px;border-radius:9px;object-fit:cover;border:1px solid var(--border);flex-shrink:0;" onclick="openLightbox(\''+r.photo_url+'\',\'Bag Photo\')" onerror="this.style.display=\'none\'">'
      :'<div style="width:44px;height:44px;border-radius:9px;background:var(--sage);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;">B</div>')
      ...
      +'<div style="margin-top:8px;padding-top:6px;border-top:1px solid var(--border);font-size:11px;color:var(--ink3);">In: '+fd(r.time_in)+' · By: '+r.handled_by+'</div>'
      +(CR!=='gm'?'<div style="display:flex;gap:6px;margin-top:8px;" onclick="event.stopPropagation();">'
        ...
        +'<button class="btn btn-outline btn-xs" onclick="openAddBags(\''+r.id+'\')">+ Bags</button>'
  ...
  if(!outs.length){co.innerHTML='<div class="es" style="padding:24px;"><div class="ei">-</div><p>No bags released yet</p></div>';}
    ...
    co.innerHTML=outs.map(function(r){
      ...
      +'<div style="display:flex;align-items:flex-start;gap:10px;">'
      +(r.photo_url?'<img src="'+r.photo_url+'" style="width:44px;height:44px;border-radius:9px;object-fit:cover;border:1px solid var(--border);flex-shrink:0;opacity:.6;" onclick="openLightbox(\''+r.photo_url+'\',\'Bag Photo\')" onerror="this.style.display=\'none\'">'
      :'<div style="width:44px;height:44px;border-radius:9px;background:var(--offwhite);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;opacity:.5;">B</div>')
      +'<div style="flex:1;min-width:0;">'
      +'<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;"><span class="tag-code" style="font-size:11px;opacity:.7;">'+(r.tag_code||'NO TAG')+'</span><span class="badge bgr" style="font-size:9px;">collected</span></div>'
      +'<div style="font-size:13px;font-weight:600;">'+r.guest_name+'</div>'
      +'<div style="font-size:11px;color:var(--ink3);margin-top:1px;">'+(r.room_number?'Room '+r.room_number+' · ':'Day visitor · ')+r.number_of_bags+' bag'+(r.number_of_bags>1?'s':'')+'</div>'
      +'<div style="margin-top:8px;padding-top:6px;border-top:1px solid var(--border);">'
      +'<div style="font-size:11px;color:var(--ink3);">In: '+fd(r.time_in)+'</div>'
      +'<div style="font-size:11px;color:var(--amber);margin-top:2px;">Out: '+fd(r.time_out)+'</div>'
      ...
      +(r.collected_by_staff?'<div style="margin-top:4px;font-size:10px;color:var(--ink2);">Released by: <strong>'+r.collected_by_staff+'</strong></div>':'')
      +'</div></div></div></div>';
    }).join('');
  }
}
function searchLug(q){
  ...
  if(!res.length){tb.innerHTML='<tr><td colspan="10"><div class="es"><p>No results</p></div></td></tr>';return;}
  tb.innerHTML=res.map(function(r){
    ...
    +'<td>'+r.handled_by+'</td><td>'+(r.collected_by_staff||'-')+'</td>'
...
function clearLugFilters(){
  document.getElementById('lug-search').value='';document.getElementById('lug-date').value='';
  document.getElementById('lug-search-results').style.display='none';renderSplit(allLug);
}
...
async function showTagDetail(id){
  ...
  var logData=logs.data||[];var od=isLugOD(r);
  ...
  if(r.photo_url)html+='<div><img src="'+r.photo_url+'" style="width:100%;max-height:200px;object-fit:cover;border-radius:var(--r);border:1px solid var(--border);" onclick="window.open(\''+r.photo_url+'\',\'_blank\')"></div>';
  html+='<div>';
  ...
  html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Tag Code</span><span class="tag-code">'+(r.tag_code||'-')+'</span></div>';
  html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Status</span><span class="badge '+(r.status==='stored'?'bg':'bgr')+'">'+r.status+'</span></div>';
  html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Guest</span><strong>'+r.guest_name+'</strong></div>';
  html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Room</span>'+(r.room_number||'Day visitor')+'</div>';
  html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Bags Now</span><strong style="font-size:16px;color:var(--green);">'+r.number_of_bags+'</strong></div>';
  html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Checked In By</span>'+r.handled_by+'</div>';
  html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Time In</span>'+fd(r.time_in)+'</div>';
  if(r.time_out)html+='<div><span style="font-size:10px;color:var(--ink3);display:block;margin-bottom:2px;text-transform:uppercase;">Time Out</span>'+fd(r.time_out)+'</div>';
  ...
  html+='<div class="tag-log-item"><div class="tag-log-ico in">In</div><div style="flex:1;"><div style="font-size:12px;font-weight:600;color:var(--ink2);">Initial Check-in - '+r.number_of_bags+' bag'+(r.number_of_bags>1?'s':'')+'</div><div style="font-size:11px;color:var(--ink3);">'+(r.bag_description||'No description')+'</div><div style="font-size:11px;color:var(--ink3);margin-top:2px;">By: '+r.handled_by+' · '+fd(r.time_in)+'</div></div></div>';
  if(logData.length){
    html+=logData.map(function(l){
      ...
      return '<div class="tag-log-item"><div class="tag-log-ico '+ico+'" style="color:'+color+';">'+l.action.charAt(0).toUpperCase()+'</div><div style="flex:1;"><div style="font-size:12px;font-weight:600;color:var(--ink);">'+label+'</div>'+(l.notes?'<div style="font-size:11px;color:var(--ink3);">'+l.notes+'</div>':'')+'<div style="font-size:11px;color:var(--ink3);margin-top:2px;">By: '+(l.staff_name||'-')+(l.collected_by?' · Collected by: '+l.collected_by:'')+' · '+fd(l.created_at)+'</div></div><div style="font-size:12px;font-weight:600;color:var(--ink3);">'+(l.bags_after!==undefined?l.bags_after+' left':'')+'</div></div>';
    }).join('');
  }
  html+='</div>';
...
function openAddBags(id){
  ...
  clr(['ab-count','ab-desc','ab-staff']);openM('m-add-bags');
}
async function addBagsToTag(){
  var id=document.getElementById('ab-lug-id').value;
  var count=parseInt(g('ab-count')||'0'),desc=g('ab-desc'),staff=g('ab-staff');
...
function openRemoveBags(id){
  ...
  clr(['rb-count','rb-by','rb-staff','rb-notes']);openM('m-remove-bags');
}
async function removeBagsFromTag(){
  var id=document.getElementById('rb-lug-id').value;
  var count=parseInt(g('rb-count')||'0'),by=g('rb-by'),staff=g('rb-staff'),notes=g('rb-notes');
...
async function doCollect(){
  ...
  closeM('m-collect');toast('Luggage released - signed off by '+CU.full_name);
...
function renderComp(rows){
  ...
  if(!rows||!rows.length){tb.innerHTML='<tr><td colspan="9"><div class="es"><p>No complaints yet</p></div></td></tr>';return;}
  tb.innerHTML=rows.map(function(r){
    var canDel=CR==='admin';
    var canResolve=r.status==='open'&&CR!=='gm'&&!isGR();
    ...
    +'<td style="white-space:nowrap;">'+(canResolve?'<button class="btn btn-outline btn-xs" onclick="openResolve(\''+r.id+'\')">Resolve</button> ':'')+( canDel?'<button class="btn btn-danger btn-xs" onclick="askDel(\'complaints\',\''+r.id+'\',\'Complaint from '+r.guest_name+'\')">Del</button>':'')+'</td></tr>';
  }).join('');
}
...
function renderWup(rows){
  ...
  if(!rows||!rows.length){tb.innerHTML='<tr><td colspan="8"><div class="es"><p>No wake-up calls scheduled</p></div></td></tr>';return;}
  tb.innerHTML=rows.map(function(r){
    var canDel=CR==='admin';
...
async function loadLF(){
  ...
  if(!r.data||!r.data.length){tb.innerHTML='<tr><td colspan="9"><div class="es"><p>No items logged yet</p></div></td></tr>';return;}
  tb.innerHTML=r.data.map(function(x){
    var canDel=CR==='admin';
    return '<tr><td>'+(x.photo_url?'<img src="'+x.photo_url+'" style="width:36px;height:36px;border-radius:7px;object-fit:cover;border:1px solid var(--border);cursor:pointer;" onclick="openLightbox(\''+x.photo_url+'\',\'Found Item\')" onerror="this.style.display=\'none\'">'
...
async function loadStore(){
  ...
  allStore=r.data||[];renderStore(allStore);renderTodayCOs();renderODStore();
...
function renderStore(rows){
  ...
  if(!rows||!rows.length){tb.innerHTML='<tr><td colspan="11"><div class="es"><p>No items in store yet</p></div></td></tr>';return;}
  var cb={excellent:'bg',good:'bg',fair:'bo',poor:'br2'};
  tb.innerHTML=rows.map(function(r){
    var od=isStoreOD(r);var canDel=CR==='admin';
    return '<tr>'
    +'<td><div style="display:flex;align-items:center;gap:8px;">'+(r.photo_url?'<img src="'+r.photo_url+'" style="width:34px;height:34px;border-radius:7px;object-fit:cover;border:1px solid var(--border);flex-shrink:0;cursor:pointer;" onclick="openLightbox(\''+r.photo_url+'\',\''+r.item_name+'\')">'
    :'<div style="width:34px;height:34px;border-radius:7px;background:var(--sagel);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:15px;">S</div>')+'<div><div style="font-size:13px;font-weight:600;">'+r.item_name+'</div>'+(od?'<span style="background:var(--redbg);color:var(--red);font-size:9px;font-weight:600;padding:1px 6px;border-radius:4px;border:1px solid var(--redbd);">Overdue</span>':'')+'</div></div></td>'
    ...
    +'<td>'+(r.owner_name||'-')+'</td><td>'+(r.room_number||'-')+'</td>'
...
function renderTodayCOs(){
  ...
  if(!rows.length){el.innerHTML='<div class="es" style="padding:16px;"><p>No checkouts today</p></div>';return;}
  el.innerHTML=rows.map(function(r){return '<div style="display:flex;align-items:center;gap:9px;padding:8px 0;border-bottom:1px solid var(--border);"><div style="flex:1;"><div style="font-size:12px;font-weight:600;">'+r.item_name+'</div><div style="font-size:11px;color:var(--ink3);">By: '+(r.checked_out_by||'-')+'</div></div><span class="tag-code" style="font-size:10px;">'+(r.tag_code||'-')+'</span></div>';}).join('');
}
function renderODStore(){
  ...
  if(!rows.length){el.innerHTML='<div class="es" style="padding:16px;"><p>No overdue items</p></div>';return;}
  el.innerHTML=rows.map(function(r){return '<div style="display:flex;align-items:center;gap:9px;padding:8px 0;border-bottom:1px solid var(--border);"><div style="flex:1;"><div style="font-size:12px;font-weight:600;color:var(--red);">'+r.item_name+'</div><div style="font-size:11px;color:var(--ink3);">'+(r.owner_name||'Hotel')+' · Since '+fd(r.date_in||r.created_at)+'</div></div><span class="tag-code" style="font-size:10px;">'+(r.tag_code||'-')+'</span></div>';}).join('');
}
function filtStore(){
...
function openCO(id){
  ...
  document.getElementById('co-info').innerHTML='<div style="display:flex;gap:10px;align-items:center;">'+(r.photo_url?'<img src="'+r.photo_url+'" style="width:44px;height:44px;border-radius:8px;object-fit:cover;border:1px solid var(--border);flex-shrink:0;">':'<div style="width:44px;height:44px;border-radius:8px;background:var(--sagel);display:flex;align-items:center;justify-content:center;font-size:20px;">S</div>')+'<div><div style="font-weight:600;font-size:13px;">'+r.item_name+'</div><div style="font-size:11px;color:var(--ink3);">Tag: <span class="tag-code" style="font-size:10px;">'+(r.tag_code||'-')+'</span> · '+r.quantity+' item(s)</div></div></div>';
...
async function loadHandovers(){
  ...
  allHandovers=r.data||[];renderHandovers(allHandovers);renderLatestHO();checkUnreadHO();
}
async function checkUnreadHO(){
  if(!allHandovers.length)return;
  var h=allHandovers[0];var reads=h.read_by||[];
...
function renderHandovers(rows){
  ...
  feed.innerHTML=sorted.map(function(h){
    ...
    var canDel=CR==='admin';
    return '<div style="padding:16px 20px;border-bottom:1px solid var(--border);'+bg+'" onclick="readHO(\''+h.id+'\')">'
    ...
    +'<div style="font-size:11px;color:var(--ink3);margin-top:8px;">By: <strong>'+h.written_by+'</strong> to <strong>'+h.incoming_staff+'</strong></div></div>'
...
async function loadUsers(){
  ...
  if(!r.data||!r.data.length){list.innerHTML='<div class="es"><p>No users yet</p></div>';return;}
...
async function addUser(){
  ...
  if(session&&session.data&&session.data.session){await db.auth.setSession({access_token:session.data.session.access_token,refresh_token:session.data.session.refresh_token});}
  suc.textContent='Added successfully!';
...
function askDel(table,id,desc){
  document.getElementById('del-table').value=table;document.getElementById('del-id').value=id;document.getElementById('del-desc').textContent=desc;openM('m-del');
}
...
function previewPhoto(input,prevId,type){
  ...
  img.onload=function(){
    ...
    var prev=document.getElementById(prevId);prev.src=compressed;prev.style.display='block';
    if(type==='lug')photoLug=compressed;
    else if(type==='lf')photoLF=compressed;
    else if(type==='store')photoStore=compressed;
    URL.revokeObjectURL(url);toast('Photo added');
  };img.src=url;
}
...
async function loadAnalytics(){
  ...
  var from=new Date();from.setDate(from.getDate()-days);var fromStr=from.toISOString();
  ...
  document.getElementById('an-lug').textContent=lD.length;document.getElementById('an-comp').textContent=cD.length;
  document.getElementById('an-wup').textContent=wD.length;document.getElementById('an-lf').textContent=lfD.length;
  ...
  drawChart('chart-complaints','doughnut',['Urgent','Medium','Low'],[urgCount,medCount,lowCount],['#c0392b','#c9922a','#2d5a1b'],'Complaints');
  var lugByDay={};
  ...
  var days2=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];var dayCounts=[0,0,0,0,0,0,0];
  ...
  drawChart('chart-days','bar',days2,dayCounts,['#4a8a2a'],'Activity by Day');
  drawChart('chart-overview','bar',['Luggage','Complaints','Wake-up','Lost & Found'],[lD.length,cD.length,wD.length,lfD.length],['#2d5a1b','#c0392b','#c9922a','#1a5fa8'],'Overview');
  var words={};
  ...
  if(!sorted.length){el.innerHTML='<div class="es"><p>Not enough complaint data yet</p></div>';return;}
...
function drawChart(id,type,labels,data,colors,label){
  ...
  var datasets=type==='doughnut'?[{data:data,backgroundColor:colors,borderWidth:2,borderColor:'#fff'}]:
    type==='line'?[{label:label,data:data,borderColor:colors[0],backgroundColor:colors[0]+'22',tension:.4,fill:true,pointRadius:3}]:
    [{label:label,data:data,backgroundColor:colors.length>1?colors:colors[0]+'cc',borderRadius:6}];
  chartInstances[id]=new Chart(ctx,{type:type,data:{labels:labels,datasets:datasets},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:type==='doughnut',position:'bottom',labels:{font:{size:11},padding:12}}},scales:type!=='doughnut'?{y:{beginAtZero:true,grid:{color:'rgba(0,0,0,.06)'},ticks:{font:{size:11}}},x:{grid:{display:false},ticks:{font:{size:11}}}}:{}}});
}
var timetablePhoto=null;var allTimetables=[];
function previewTimetable(input){
  ...
  img.onload=function(){
    ...
    var prev=document.getElementById('tt-photo-prev');prev.src=timetablePhoto;prev.style.display='block';
    URL.revokeObjectURL(url);toast('Timetable photo ready');
  };img.src=url;
}
async function saveTimetable(){
  ...
  if(r.error){toast('Error: '+r.error.message,'error');return;}
  toast('Timetable uploaded');clr(['tt-month','tt-notes']);
  document.getElementById('tt-photo-prev').style.display='none';timetablePhoto=null;loadTimetable();
}
async function loadTimetable(){
  var r=await db.from('timetables').select('*').order('created_at',{ascending:false}).limit(10);
  allTimetables=r.data||[];var data=allTimetables;
  ...
  if(!data.length){
    if(ttDisplay)ttDisplay.innerHTML='<div class="es"><p>No timetable uploaded yet</p></div>';
    if(ttHistory)ttHistory.innerHTML='<div class="es"><p>No previous timetables</p></div>';
    if(ttDashPreview)ttDashPreview.style.display='none';return;
  }
  var current=data[0];
  ...
  if(ttDisplay){
    ...
    html+='</div><button class="btn btn-outline btn-sm" onclick="openLightbox(allTimetables[0].photo_url,\'Timetable\')">View Full Size</button></div>';
  ...
  if(ttHistory){
    ...
    ttHistory.innerHTML=history.map(function(t,idx){
      ...
      var h2='<div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--border);">';
      h2+='<img src="'+t.photo_url+'" style="width:60px;height:60px;object-fit:cover;border-radius:8px;border:1px solid var(--border);cursor:pointer;flex-shrink:0;" onclick="openLightbox(allTimetables['+tidx+'].photo_url,\'Previous Timetable\')">';
      h2+='<div style="flex:1;"><div style="font-size:13px;font-weight:600;">'+ml+'</div>';
      h2+='<div style="font-size:11px;color:var(--ink3);">'+t.uploaded_by+' · '+fd(t.created_at)+'</div>';
      if(t.notes)h2+='<div style="font-size:11px;color:var(--ink3);">'+t.notes+'</div>';
      h2+='</div><button class="btn btn-outline btn-xs" onclick="openLightbox(allTimetables['+tidx+'].photo_url,\'Previous Timetable\')">View</button>';
      if(CR==='admin'){h2+='<button class="btn btn-danger btn-xs" data-id="'+t.id+'" onclick="delTimetable(this.dataset.id)">Del</button>';}
      h2+='</div>';return h2;
    }).join('');
  }
}
...
function resetNpDate(){document.getElementById('np-date').value=new Date().toISOString().split('T')[0];loadNpDay();}
async function loadNpDay(){
  var date=document.getElementById('np-date').value;if(!date)return;
...
function renderNpChecklist(){
  var html='<div style="padding:10px 16px 6px;display:flex;gap:16px;background:var(--sagel);border-bottom:1px solid var(--border);overflow-x:auto;">';
  ...
  ALL_PAPERS.forEach(function(p){html+='<div style="font-size:10px;font-weight:700;color:var(--ink3);text-transform:uppercase;letter-spacing:.8px;min-width:90px;">'+p+'</div>';});
  html+='</div>';
  NP_RECIPIENTS.forEach(function(rec){
    html+='<div class="np-check-row"><div class="np-name">'+rec.name+'</div><div class="np-papers">';
...
async function saveNpDay(){
  ...
  var res;
  if(r.data){res=await db.from('newspaper_deliveries').update(payload).eq('id',r.data.id);}
  else{res=await db.from('newspaper_deliveries').insert([payload]);}
...
async function genNpReport(){
  ...
  if(!month){toast('Please select a month','error');return;}
  var from=month+'-01';
  var toDate=new Date(month+'-01');toDate.setMonth(toDate.getMonth()+1);toDate.setDate(0);
  ...
  if(!rows.length){out.innerHTML='<div class="es"><p>No data for '+monthLabel+'</p></div>';return;}
  var totals={};
  NP_RECIPIENTS.forEach(function(rec){totals[rec.id]={};ALL_PAPERS.forEach(function(paper){totals[rec.id][paper]=0;});});
  rows.forEach(function(row){
    var d=row.deliveries||{};
    NP_RECIPIENTS.forEach(function(rec){ALL_PAPERS.forEach(function(paper){var key=rec.id+'_'+paper.replace(/ /g,'_');if(d[key]===true)totals[rec.id][paper]++;});});
  });
  var now2=new Date().toLocaleString('en-GB',{day:'2-digit',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'});
  ...
  NP_RECIPIENTS.forEach(function(rec){
    var rowTotal=0;html+='<tr><td><strong>'+rec.name+'</strong></td>';
    ALL_PAPERS.forEach(function(paper){
      var allowed=rec.papers.indexOf(paper)>-1;var count=totals[rec.id][paper];rowTotal+=count;
      if(allowed){html+='<td style="text-align:center;"><strong style="color:'+(count>0?'var(--green)':'var(--ink3)')+';">'+count+'</strong></td>';}
      else{html+='<td style="text-align:center;color:var(--border);">-</td>';}
    });
    html+='<td style="text-align:center;"><span class="badge '+(rowTotal>0?'bg':'bgr')+'">'+rowTotal+'</span></td></tr>';
  });
  html+='<tr style="background:var(--sagel);font-weight:700;"><td>TOTAL</td>';
  var grandTotal=0;
  ALL_PAPERS.forEach(function(paper){
    var colTotal=0;NP_RECIPIENTS.forEach(function(rec){colTotal+=totals[rec.id][paper];});grandTotal+=colTotal;
    html+='<td style="text-align:center;font-weight:700;">'+colTotal+'</td>';
  });
  html+='<td style="text-align:center;font-weight:700;">'+grandTotal+'</td></tr>';
...
function closeLightbox(e){
  ...
  document.getElementById('lightbox').style.display='none';document.body.style.overflow='';lbRotation=0;
}
function rotateLightbox(){lbRotation=(lbRotation+90)%360;document.getElementById('lightbox-img').style.transform='rotate('+lbRotation+'deg)';}
document.addEventListener('keydown',function(e){if(e.key==='Escape'){document.getElementById('lightbox').style.display='none';document.body.style.overflow='';}});
